...
function getStreakColor(count: number, colors: { error: string; warning: string; success: string; disabled: string }): string {
  if (count >= 30) return colors.error;    // Red, on fire
  if (count >= 7) return colors.warning;   // Amber, building
  if (count >= 1) return colors.success;   // Green, starting
  return colors.disabled;                  // Grey, no streak
}
...
export function StreakBadge({ streak, size = 'md', showFreeze = false }: StreakBadgeProps) {
  ...
  const color = getStreakColor(streak.currentStreak, colors);
