...
export function StreakBadge({ streak, size = 'md', showFreeze = false }: StreakBadgeProps) {
  ...
  const color = getStreakColor(streak.currentStreak, colors.link);
    ...
    >
      ...
      {showFreeze && streak.freezeTokens > 0 && (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
          ...
          <Text style={{ color: colors.link, fontSize: s.freezeText }}>{streak.freezeTokens}</Text>
        </View>
      )}
    </View>
  );
