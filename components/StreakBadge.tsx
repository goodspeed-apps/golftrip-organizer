...
export function StreakBadge({ streak, size = 'md', showFreeze = false }: StreakBadgeProps) {
  ...
  return (
    <View
      style={{ flexDirection: 'row', alignItems: 'center', gap: s.gap, backgroundColor: colors.info, padding: s.padding, borderRadius: s.padding + s.icon / 2 }}
      accessibilityLabel={`${streak.currentStreak} day streak`}
    >
      <Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
        <Flame size={s.icon} color={color} />
      </Animated.View>
      <Text style={{ fontSize: s.text, fontWeight: '700', color }}>
        {streak.currentStreak}
      </Text>
      {showFreeze && streak.freezeTokens > 0 && (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
          <Snowflake size={s.freezeIcon} color={colors.link} />
          <Text style={{ fontSize: s.freezeText, color: colors.link, fontWeight: '600' }}>
            {streak.freezeTokens}
          </Text>
        </View>
      )}
