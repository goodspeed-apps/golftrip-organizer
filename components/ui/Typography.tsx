...
export function Heading({ color, center, style, ...props }: TypographyProps) {
  ...
  const s: TextStyle = { color: color ?? colors.text, fontSize: 24, fontWeight: weight, textAlign: center ? 'center' : undefined };
...
export function Subheading({ color, center, style, ...props }: TypographyProps) {
  const { colors } = useThemeColors();
  const s: TextStyle = { color: color ?? colors.text, fontSize: 18, fontWeight: '600', textAlign: center ? 'center' : undefined };
  return <Text style={[s, style]} {...props} />;
}

export function Body({ color, center, style, ...props }: TypographyProps) {
  const { colors } = useThemeColors();
  const s: TextStyle = { color: color ?? colors.text, fontSize: 15, lineHeight: 22, textAlign: center ? 'center' : undefined };
  return <Text style={[s, style]} {...props} />;
}

export function Caption({ color, center, style, ...props }: TypographyProps) {
  const { colors } = useThemeColors();
  const s: TextStyle = { color: color ?? colors.textSecondary, fontSize: 12, textAlign: center ? 'center' : undefined };
  return <Text style={[s, style]} {...props} />;
}

export function Label({ color, center, style, ...props }: TypographyProps) {
  const { colors } = useThemeColors();
  const s: TextStyle = { color: color ?? colors.textSecondary, fontSize: 13, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5, textAlign: center ? 'center' : undefined };
  return <Text style={[s, style]} {...props} />;
}
