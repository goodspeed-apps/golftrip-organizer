...
export function FeedbackButton({
  ...
  const openModal = useCallback((src: 'button' | 'shake') => {
    ...
    addBreadcrumb('ui', `FeedbackButton opened via ${src}`);
  }, []);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('feedback').insert({
        ...
        app_version: gasConfig.app?.version ?? null,
      });
  ...
  }, [text, category, source, pathname, userId, subscriptionTier, closeModal]);
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            ...
            >
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                ...
                <TouchableOpacity onPress={closeModal} accessibilityLabel="Close feedback">
                    ...
                    {CATEGORIES.map((cat) => (
                      <TouchableOpacity
                        ...
                        style={{
                          ...
                          borderRadius: 20,
                          marginRight: 8,
                          backgroundColor: category === cat.id ? cat.color : borderDark,
                        }}
                        accessibilityLabel={cat.label}
                      >
                        <cat.icon size={14} color={category === cat.id ? colors.textOnPrimary : colors.textSecondary} />
                        <Text style={{ color: category === cat.id ? colors.textOnPrimary : colors.textSecondary, fontSize: 13 }}>
                          {cat.label}
                        </Text>
                      </TouchableOpacity>
                    ))}
