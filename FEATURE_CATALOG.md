...
- **native_visual_regression** - `__owl__/` directory with `owl.config.ts` and seven `.owl.ts` test files (Welcome, Signup, Login, Onboarding, Paywall, Settings, Profile). Uses `react-native-owl` for image-snapshot regression against iOS and Android simulators. Baselines committed to repo; CI via `.github/workflows/visual-native.yml` on PRs touching `app/**`, `components/**`, or `context/**`. 0.1% pixel diff tolerance. `pnpm visual:native` runs locally. Documented in `docs/VISUAL_REGRESSION.md` (Native section). Status: Static.
