...
export default function ProfileScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const rows: TripRow[] = (members ?? []).flatMap((m: { role: string; trips: { id: string; name: string; start_date: string; status: string }[] | { id: string; name: string; start_date: string; status: string } | null }) => {
        const tripsData = m.trips;
        if (!tripsData) return [];
        const tripItems = Array.isArray(tripsData) ? tripsData : [tripsData];
        return tripItems.map((t) => ({
          id: t.id ?? '',
          name: t.name ?? '',
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: c.background }}>
      <FlatList
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                ...
                <View>
                  <Text style={{ fontSize: 20, fontWeight: '700', color: c.text }}>{user?.user_metadata?.display_name ?? 'Golfer'}</Text>
            ...
            <View style={{ flexDirection: 'row', marginHorizontal: 20, marginBottom: 20, gap: 12 }}>
              {[
                ...
                <View key={s.label} style={{ flex: 1, backgroundColor: c.surface, borderRadius: 12, padding: 14, alignItems: 'center' }}>
                  ...
                  <Text style={{ fontSize: 12, color: c.textMuted, marginTop: 2 }}>{s.label}</Text>
                </View>
              ))}
            </View>

            {/* Tabs */}
            <View style={{ flexDirection: 'row', marginHorizontal: 20, marginBottom: 16, backgroundColor: c.surface, borderRadius: 10, padding: 4 }}>
              {TABS.map((tab) => (
                <Pressable
                  key={tab}
                ...
                >
                  <Text style={{ fontWeight: '600', fontSize: 14, color: activeTab === tab ? '#fff' : c.textMuted }}>{tab}</Text>
                </Pressable>
              ))}
            </View>
          </View>
        ...
        ListEmptyComponent={
          <EmptyState
            title={activeTab === 'Organizing' ? 'No trips organized yet' : 'Not on any trips yet'}
            subtitle="Trips you join or organize will appear here"
            icon={MapPin}
          />
        }
        renderItem={({ item }) => (
