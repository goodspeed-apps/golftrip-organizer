...
## Cluster 7: Native Visual Regression, Push Receipts, Screenshot Automation, Per-Platform SLO, Typed Widget Data, FormWizard (2026-05-13)

### Added - Native visual regression via react-native-owl
- `__owl__/` directory with `owl.config.ts` and seven golden-path test files covering Welcome, Signup, Login, Onboarding, Paywall, Settings, and Profile screens.
- `.github/workflows/visual-native.yml` runs the Owl suite on PRs touching `app/**`, `components/**`, or `context/**`, against both iOS and Android simulators.
- Pixel diff tolerance: 0.1% by default. Baselines are committed to the repo. Update baselines by running `pnpm visual:native --update`.
- `scripts/run-owl.mjs` boots simulators, runs Owl, and exits non-zero on any diff.
- `docs/VISUAL_REGRESSION.md` extended with a Native section alongside the existing Chromatic section.
