...
## Cluster 7: Native Visual Regression, Push Receipts, Screenshot Automation, Per-Platform SLO, Typed Widget Data, FormWizard

### Native visual regression (react-native-owl)

Full setup in `docs/VISUAL_REGRESSION.md` (Native section).

Baselines are committed to the repo under `__owl__/__snapshots__/`. The CI workflow (`.github/workflows/visual-native.yml`) runs the Owl suite on every PR that touches `app/**`, `components/**`, or `context/**`, against both iOS and Android simulators.

To update baselines after an intentional UI change:

pnpm visual:native --update

Commit the updated snapshot files alongside the code change. The default pixel diff tolerance is 0.1%; adjust in `__owl__/owl.config.ts`.
...
- Native Owl CI uses the existing EAS build secrets (`EAS_TOKEN`, `EXPO_TOKEN`) already provisioned for the cluster 3/6 workflows.
