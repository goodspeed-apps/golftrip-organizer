...
import type { RadiusToken, SpacingToken } from '../lib/design-tokens';
...
const LightColors = {
  ...
  textOnPrimary: getContrastColor(configColors.primary),
  buttonText: getContrastColor(configColors.primary),
...
const DarkColors = {
  ...
  textOnPrimary: getContrastColor(configColors.primaryDark),
  buttonText: getContrastColor(configColors.primaryDark),
...
type ThemeContextValue = {
  ...
  /** Resolve corner radius px from a RadiusToken (or the app's configured default) */
  radius: (token?: RadiusToken | string) => number;
  /** Density multiplier from a SpacingToken (or the app's configured default) */
  densityScale: (token?: SpacingToken | string) => number;
  /** Apply density scaling to a base px value, rounded */
  pad: (basePx: number) => number;
  /** Elevation / shadow style object derived from mood */
  cardShadow: (mood?: string) => ReturnType<typeof cardShadow>;
};
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ThemeContext.Provider value={{ resolved, preference, setTheme, colors, fontScale, reducedMotion, radius, densityScale, pad, cardShadow }}>
      {children}
